...
import { getSupplierBrandModelParts, partSearch, getSupplierParts } from '../../actions/parts';
...
class OrderForms extends Component {
  ...
  componentWillMount() {
    if (this.state.supplier && this.state.supplier.id) {
      this.loadBrandsBySupplier(this.state.supplier.id);
    }

    if (this.state.brand.id) this.loadBrandModels(this.state.brand.id);
  }

  loadBrandsBySupplier(supplierId) {
    this.props.getSupplierParts(supplierId).then((data) => {
      const brandIds = data.suplier.map(item => item.pr_brand_id);

      this.setState({
        brands: this.props.brands.filter(item => brandIds.includes(item.id)),
      });
    });
  }
  ...
  loadBrandModels(brandId) {
    return this.props.getBrandModels(brandId)
      .then(({ models }) => {
        this.props.getSupplierParts(this.state.supplier.id).then((data) => {
          const modelIds = data.suplier.map(item => item.md_id);

          this.setState({
            models: models.filter(item => modelIds.includes(item.id)),
          });
        });
      });
  }
  ...
  handleSupplierSelect(supplier) {
    ...
    this.loadBrandsBySupplier(supplier.id);
  }
  ...
  handleAddToListButtonPress() {
    const orderPart = {
      ...
      ord_id: this.props.order.ord_id,
      sup_id: this.state.supplier.id,
      brand_id: this.state.brand.id,
      pr_id: this.state.part.id,
    ...
    const errors = Validator.getFormErrors(orderPart, formsValidation.ORDER_PART);
    if (errors.length > 0) return Alert.showMessage(I18n.t('translation.error'), errors.join('\n'));

    this.state.order.parts.push(orderPart);
    this.setState({ order: this.state.order });
  }

  handleSaveButtonPress() {
    if (!this.state.order.parts.length) return;
    this.setState({ loading: true });

    const request = this.state.order.ord_id
      ? this.props.postOrderParts
      : this.props.postOrder;

    request(this.state.order.parts)
      .then(response => this.props.onActionsSuccess(response.order))
      .catch(() => {
  ...
  handleRemovePartPress(index) {
    this.state.order.parts.splice(index, 1);
    this.setState({ order: this.state.order });
  }
  ...
  render() {
    return (
      <Scene loading={this.state.loading}>
        ...
        <Header
          text={this.state.order.ord_id ? I18n.t('translation.addOrderParts') : I18n.t('translation.addOrder')}
        ...
        <ScrollView keyboardShouldPersistTaps="handled">
          <View style={styles.scene.responsiveWrapper}>
            <View style={styles.scene.responsive}>
              ...
              <View style={styles.scene.listItemsContainer}>
                {this.state.order.parts.map((part, id) => this.renderOrderPartsListItem(part, id))}
              </View>
            </View>
          </View>
        </ScrollView>
  ...
  static propTypes = {
    ...
    postOrderParts: PropTypes.func.isRequired,

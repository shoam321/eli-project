...
class Validator {
  ...
  getErrorMessage() {
    return `${I18n.t(this.name)} ${I18n.t(this.validator.error.key, this.validator.error.data)}`;
  }

  static getFormErrors(formData, formValidator) {
    return Object.keys(formValidator).reduce((errors, key) => {
      if (!formValidator[key].validate(formData[key])) return [...errors, formValidator[key].getErrorMessage()];
      return errors;
    }, []);
  }
  ...
  static integerPositive = {
    required: {
      func: value => Number.isInteger(Number.parseInt(value)) && Number.parseInt(value) >= 0,
      error: {
  ...
  static email = {
    optional: {
      func: value => !(value || '').length || Validator.regex.email.test((value || '').toLowerCase()),
      error: {
        key: 'translation.isInvalid',
      },
    },
    required: {
      func: value => (value || '').length > 0 && Validator.regex.email.test((value || '').toLowerCase()),
      error: {
        key: 'translation.isInvalid',
      },
    },
  ...
  static phone = {
    ...
    required: {
      func: value => value.length && Validator.regex.phone.test(value.toLowerCase()),
      error: {
  ...
  static imei = {
    required: {
      func: (value = '') => !value.length || Validator.regex.imei.test(value.toString()),
      error: {
...
export const formsValidation = {
  ...
  PROJECT: {
    ...
    device_id: new Validator(Validator.integer.required, 'translation.clientDevice'),
    ...
    signature_image: new Validator(Validator.image.required, 'translation.clientSignature'),
  },
  CLIENT: {
    ...
    secondary_phone: new Validator(Validator.phone.optional, 'translation.secondaryPhone'),
  },
  CLIENT_MIN: {
    name: new Validator(Validator.string.required, 'translation.name'),
    last_name: new Validator(Validator.string.required, 'translation.lastName'),
  ...
  SUPPLIER: {
    sup_name: new Validator(Validator.string.required, 'translation.name'),
    sup_email: new Validator(Validator.email.optional, 'translation.email'),
    sup_phone: new Validator(Validator.phone.optional, 'translation.phone'),
    secondary_phone: new Validator(Validator.phone.optional, 'translation.secondaryPhone'),
  },
  PART: {
    ...
    in_stock: new Validator(Validator.integerPositive.required, 'translation.stock'),
    price: new Validator(Validator.integerPositive.required, 'translation.price'),
  },
  ORDER_PART: {
    sup_id: new Validator(Validator.integer.required, 'translation.supplier'),
  ...
  CART_PART: {
    supplier_id: new Validator(Validator.integer.required, 'translation.supplier'),
  ...
  SELL_DEVICE: {
    ...
    warranty_expiration_date: new Validator(Validator.string.required, 'translation.warrantyExpirationDate'),
    actual_sell_price: new Validator(Validator.integerPositive.required, 'translation.actualSellPrice'),
    life_status: new Validator(Validator.integer.required, 'translation.status'),
  },
  SELL_PART: {
    user_id: new Validator(Validator.integer.required, 'translation.client'),
    warranty_expiration_date: new Validator(Validator.string.required, 'translation.warrantyExpirationDate'),
  ...
  ACCOUNT: {
    ac_email: new Validator(Validator.string.required, 'translation.email'),
    ac_pass: new Validator(Validator.string.required, 'translation.password'),
  },
  BRAND: {
    name: new Validator(Validator.string.required, 'translation.name'),
  },

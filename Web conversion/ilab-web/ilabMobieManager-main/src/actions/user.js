...
export const postFcmToken = (dev_key) => (/* dispatch */) =>
  restClient
    .getAxios()
    .post(remote.api, { dev_key, tag: 'add_user_dev_key' })
...
export const getAppLanguage = () => () =>
  AsyncStorage.getItem('app_language').then((data) => {
    ...
    if (data) {
      ...
      data =
        appLanguages.find(
          (appLanguage) => appLanguage.code === deviceLanguage
        ) || appLanguages[0];
    }
...
export const getSavedLogin = () => (/* dispatch */) =>
  AsyncStorage.getItem('_login').then((login) => {
    if (!login) return Promise.reject('Login is null');
    return Promise.resolve(login);
  });

export const saveLogin = (login) => (/* dispatch */) =>
  AsyncStorage.setItem('_login', login);

export const logIn = (credentials) => (/* dispatch */) =>
  restClient
    .getAxios()
    .post(remote.api, { ...credentials, tag: 'login_user' })
    .then(({ data }) => {
      console.log(data);
...
export const getUser = () => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'get_user_info' })
    .then(({ data }) => {
...
export const updateCurrentUser = (data) => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'update_current_user', data })
    .then(({ data }) => {
      dispatch({ type: userTypes.USER_LOADED, payload: data.user });
      return Promise.resolve(data);
    })
...
export const logOut = () => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'logout_user' })
    .then((response) => {

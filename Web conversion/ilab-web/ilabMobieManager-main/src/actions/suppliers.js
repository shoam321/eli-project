...
export const supplierSearch = (supplier, string) => {
  ...
  return (
    supplier.sup_name.search(regexp) >= 0 ||
...
export const getSuppliers = () => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'get_company_supliers' })
    .then(({ data }) => {
      dispatch({
        type: mainTypes.DATA_LOADED,
        payload: data.suplier,
        key: 'suppliers',
      });
...
export const postSupplier = (supplier) => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'add_suplier', suplier: supplier })
    .then((response) => {
...
export const updateSupplier = (supplier) => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'update_suplier', suplier: supplier })
    .then((response) => {
      getSuppliers()(dispatch);
...
export const deleteSupplier = (id) => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'delete_suplier', sup_id: id })

...
export const cartPartSearch = (part, string) => {
  ...
  return part.pr_name.search(regexp) >= 0 || part.br_name.search(regexp) >= 0;
};

export const getCartItems = () => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'get_cart_items' })
    .then(({ data }) => {
      dispatch({
        type: mainTypes.DATA_LOADED,
        payload: data.card_items,
        key: 'carts',
      });
...
export const addCartItem = (item) => (dispatch) =>
  restClient
    .getAxios()
    .post(remote.api, { tag: 'add_cart_item', item })
    .then(({ data }) => {
